...
export class img
{
  ...
  reg(name: string, url: string, upload: (file: Buffer, fileName: string) => Promise<string>)
  {
    ...
    {
      this.ctx.logger.info(`注册图床服务: ${name}`);
      this.regList[name] = {
        url: url,
        upload: upload
      }
    ...
    {
      this.ctx.logger.warn(`图床服务 ${name} 已经注册过了，无法重复注册。`);
      return false;
    }
  }
  ...
  async speedTest()
  {
    ...
    {
      this.ctx.logger.error(`所有图床服务测试失败`);
      return;
    }

    this.ctx.logger.info(`图床服务测试完成，最快的服务是: ${fastName}`);

    this.fastName = fastName; // 设置最快的图床服务名称
